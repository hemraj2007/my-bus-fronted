...
export default function Profile() {
  ...
  // Profile update karne wali function
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/users/update_profile`, {
        ...
        body: JSON.stringify({
          name: profile.name,
          email: profile.email, // Email ko update karne nahi denge, agar edit mode hai
          mob_number: profile.mob_number,
        }), // Update kar rahe hain name, email, aur mob_number
      });
  ...
  return (
    <div className="flex flex-col md:flex-row items-start justify-center min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6 space-y-6 md:space-y-0 md:space-x-10">
      ...
      <div className="bg-white shadow-xl rounded-xl p-8 w-full md:max-w-sm">
        ...
        <p className="text-gray-700 mt-2"><strong>Mobile Number:</strong> {profile.mob_number}</p>
      ...
      {editMode && (
        <div className="bg-white shadow-xl rounded-xl p-8 w-full md:max-w-sm">
          <h2 className="text-2xl font-bold text-indigo-600 mb-4">✏️ Edit Profile</h2>
          <form onSubmit={handleUpdate} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                value={profile.email}
                disabled
                className="border border-gray-300 p-3 rounded-lg w-full bg-gray-200 cursor-not-allowed"
              />
            </div>
